...
class PlayerControls(QWidget):
    ...
    render_backend_changed = Signal(str)
    ...
    def _setup_ui(self):
        ...
        self.render_button = QPushButton("CPU")
        self.render_button.setFixedSize(54, 40)
        self.render_button.setToolTip("Switch visual renderer: CPU / GPU")
        self.render_button.setStyleSheet(self._button_style("#305047"))
        self.render_button.clicked.connect(self._on_render_backend_clicked)
    ...
    def _on_render_backend_clicked(self):
        """Toggle between CPU and GPU visual renderers."""
        next_backend = "gpu" if self._render_backend == "cpu" else "cpu"
        self.render_backend_changed.emit(next_backend)
    ...
    def set_render_backend(self, backend: str, gpu_available: bool = True):
        """Update the renderer toggle display."""
        self._render_backend = "gpu" if backend == "gpu" and gpu_available else "cpu"
        self.render_button.setText(self._render_backend.upper())
        self.render_button.setEnabled(gpu_available)
        if gpu_available:
            self.render_button.setToolTip("Switch visual renderer: CPU / GPU")
        else:
            self.render_button.setToolTip("GPU renderer is unavailable in this environment")
