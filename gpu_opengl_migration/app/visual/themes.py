...
SCIENTIFIC_PALETTE_BANKS: Dict[str, List[str]] = {
    "nature_signal": ["#E64B35", "#4DBBD5", "#00A087", "#3C5488", "#F39B7F"],
    "nature_calm": ["#8491B4", "#91D1C2", "#7E6148", "#B09C85", "#6F99AD"],
    "science_bold": ["#BC3C29", "#0072B5", "#E18727", "#20854E", "#7876B1"],
    "clinical_neon": ["#00468B", "#0099B4", "#42B540", "#925E9F", "#FFDC91"],
    "data_ember": ["#DC0000", "#ED0000", "#EE4C97", "#FF6B6B", "#FFB347"],
    "slate_lab": ["#0B1420", "#132238", "#1B314F", "#26496B", "#345C86"],
}
...
class Theme:
    ...
    def _generate_scientific_palette(self) -> Dict[str, ColorRGB]:
        ...
        primary_rgb = mix_colors(
            [seed_primary, seed_secondary, seed_accent],
            [1.0, blend_t, accent_t],
        )
        primary_rgb = tune_color(
            primary_rgb,
            hue_shift=(f.high_ratio - f.bass_ratio) * 22.0,
            saturation_scale=1.08 + f.spectral_contrast * 0.16,
            lightness_shift=-0.03 + f.brightness * 0.07,
        )

        secondary_rgb = mix_colors(
            [
                seed_secondary,
                tune_color(primary_rgb, hue_shift=16.0 + f.warmth * 8.0, saturation_scale=0.92, lightness_shift=0.03),
                seed_accent,
            ],
            [1.0, 0.55, 0.25 + f.chaos * 0.18],
        )
        secondary_rgb = tune_color(
            secondary_rgb,
            hue_shift=-8.0 + f.mid_ratio * 12.0,
            saturation_scale=0.92 + f.energy * 0.10,
            lightness_shift=0.01,
        )

        accent_rgb = mix_colors(
            [
                seed_accent,
                tune_color(primary_rgb, hue_shift=32.0 - f.bass_ratio * 18.0, saturation_scale=1.18, lightness_shift=0.13),
                (245, 248, 255),
            ],
            [1.0, 0.72, 0.22 + f.brightness * 0.20],
        )
        accent_rgb = tune_color(
            accent_rgb,
            saturation_scale=1.08 + f.energy * 0.10,
            lightness_shift=0.02 + f.brightness * 0.04,
        )
